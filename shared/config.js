...
// Returns 80 if --largeserver flag, otherwise 50 (aligned with stable private servers)
...
function resolveSandboxFlag() {
    if (!hasProcess) return false;
    var env = process.env;
    if (!env) return false;
    return env.MOOMOO_SANDBOX === "1" || env.MOHMOH_SANDBOX === "1";
}
...
export default defineConfig(groupedConfig);
