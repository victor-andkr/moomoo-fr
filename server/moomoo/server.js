...
export class Game {
    ...
    constructor() {
        ...
        this.ai_manager = new AiManager(this.ais, AI, this.players, items, this.object_manager, config, UTILS, (player, score) => {
            if (!player?.addResource || !player.earnXP) return;
            player.addResource(3, score);
            player.earnXP(score);
        }, this.server);
    ...
    pickTurretTarget(structure) {
        ...
        for (const player of this.players) {
            ...
            const empHelmet = config.specialItems?.empHelmetID ?? 22;
            if (player.skinIndex === empHelmet) continue;
