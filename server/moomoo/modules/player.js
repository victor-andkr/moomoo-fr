...
export class Player {
    ...
    constructor(id, sid, config, UTILS, projectileManager, objectManager, players, ais, items, hats, accessories, socket, scoreCallback, iconCallback) {
        ...
        this.addProjectile = () => {
            const tur = this.skin?.turret;
            if (!tur) return;

            const empId = config.specialItems?.empHelmetID ?? 22;
            const maxRange = config.turret?.targetRange ?? 735;
            const projSpeed = config.turret?.projectileSpeed ?? 1.5;
            ...
            for (const pl of players) {
                if (
                    ...
                    pl.skinIndex !== empId &&
                    pl !== this &&
                    !(this.team && this.team === pl.team)
                ) {
                    const d = UTILS.getDistance(this.x, this.y, pl.x, pl.y);
                    if (d <= tur.range && (!nearest || d < bestDst)) {
                        bestDst = d;
                        nearest = pl;
                    }
                }
            ...
            for (const ai of ais) {
                if (ai.alive && ai.active && ai.hostile) {
                    const d = UTILS.getDistance(this.x, this.y, ai.x, ai.y);
                    if (d <= tur.range && (!nearest || d < bestDst)) {
                        bestDst = d;
                        nearest = ai;
                    }
            ...
            if (!nearest) return;
            if (UTILS.getDistance(this.x, this.y, nearest.x, nearest.y) > maxRange) return;

            this.shootCount = tur.rate ?? config.turret?.fireRate ?? 2500;
            const dir = UTILS.getDirection(nearest.x, nearest.y, this.x, this.y);
